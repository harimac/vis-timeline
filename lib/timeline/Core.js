...
class Core {
    ...
    _create(container) {
        ...
        this.dom.groupShrinkBtn = document.createElement('div'); // [XAM] collapsible group tree
        ...
        this.dom.groupShrinkBtn.className = 'vis-group-shrink-btn'; // [XAM] collapsible group tree
        ...
        this.dom.root.appendChild(this.dom.groupShrinkBtn); // [XAM] collapsible group tree
        ...
        function onMouseWheel(event) {
            ...
            // [XAM]-S Comment out for enabling both horizontal and vertical scroll
            ...
            } else*/ if (this.options.horizontalScroll) { // [XAM]-E Comment out
        ...
        // [XAM]-S collapsible group tree
        this.dom.shrinkGroup = false;
        this.dom.groupShrinkBtn.addEventListener('click', this.onClickGroupShrinkButton.bind(this));
        // [XAM]-E collapsible group tree
    ...
    setOptions(options) {
        if (options) {
            ...
            const fields = [
                ...
                'groupShrinked', 'onShrinkGroup'  // [XAM] collapsible group tree
            ];
            util.selectiveExtend(fields, this.options, options);
            this.dom.groupShrinkBtn.style.visibility = 'show'; // [XAM] collapsible group tree
            ...
            // [XAM]-S collapsible group tree
            if (options.groupShrinked) {
                this.dom.shrinkGroup = true;
                if (this.dom.shrinkGroup) {
                    if (this.options.rtl) {
                        this.dom.rightContainer.style.width = '24px';
                    }
                    else {
                        this.dom.leftContainer.style.width = '24px';
                    }
                }
                else {
                    if (this.options.rtl) {
                        this.dom.rightContainer.style.width = null;
                    }
                    else {
                        this.dom.leftContainer.style.width = null;
                    }
                }
            }
            // [XAM]-E collapsible group tree
    ...
    // [XAM]-S collapsible group tree
    /** Click handler of group collapse button
     * @param {Object} e event arguments
     */
    onClickGroupShrinkButton(e) {
        e.preventDefault();
        this.dom.shrinkGroup = !this.dom.shrinkGroup;
        if (this.dom.shrinkGroup) {
            if (this.options.rtl) {
                this.dom.rightContainer.style.width = '24px';
            }
        ...
        if (typeof(this.options.onShrinkGroup) === "function") {
            this.options.onShrinkGroup(this.dom.shrinkGroup);
        }
        this.redraw();
    }
    // [XAM]-E collapsible group tree
    ...
    _setDOM() {
        ...
        // [XAM]-S collapsible group tree
        let hiddenByTB = false, hiddenByRL = false;
        if (props.top.height > 0) {
            dom.groupShrinkBtn.style.top = `${props.top.height - 24}px`;
            dom.groupShrinkBtn.style.bottom = null;
        }
        else if (props.bottom.height > 0) {
            dom.groupShrinkBtn.style.top = null;
            dom.groupShrinkBtn.style.bottom = `${props.bottom.height - 24 - props.border.bottom}px`;
        }
        else {
            hiddenByTB = true;
        }
        if (props.left.width > 0) {
            dom.groupShrinkBtn.style.left = '0px';
            dom.groupShrinkBtn.style.right = null;
        }
        else if (props.right.width > 0) {
            dom.groupShrinkBtn.style.left = null;
            dom.groupShrinkBtn.style.right = `${props.right.width - 24 - props.border.right}px`;
        }
        else {
            hiddenByRL = true;
        }

        if (hiddenByTB || hiddenByRL) {
            dom.groupShrinkBtn.style.visibility = 'hidden';
        }
        else {
            dom.groupShrinkBtn.style.visibility = 'visible';
        }
        // [XAM]-E collapsible group tree
    }
