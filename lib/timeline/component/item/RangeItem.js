 ...
 */
  ...
  _createDomElement() {
    if (!this.dom) {
      ...
      // [XAM]-S Make content draw on visibleFrame without z-index option (Swap Order to appendChild)
  ...
  repositionX(limitSize) {
    ...
    if (this.options.rtl) {
      // [XAM]-S for using left instead of tranlateX
      //this.dom.box.style.transform = `translateX(${this.right * -1}px)`;
      if (this.right) {
        this.dom.box.style.left = `${this.right * -1}px`;
      }
      else {
        this.dom.box.style.left = null;
      }
      // [XAM]-E
    } else {
      // [XAM]-S for using left instead of tranlateX
    ...
    switch (align) {
      case 'left':
        ...
        this.dom.content.style.left = null;
        // [XAM]-E
        break;
      ...
      case 'right':
        if (this.options.rtl) {
          ...
          if (translateX) {
            this.dom.content.style.left = `${translateX}px`;
          }
          else {
            this.dom.content.style.left = null;
          }
          ...
          const translateX = Math.max((boxWidth - contentWidth), 0);
          if (translateX) {
            this.dom.content.style.left = `${translateX}px`;
          }
          else {
      ...
      case 'center':
        if (this.options.rtl) {
          ...
          const translateX = Math.max((boxWidth - contentWidth) / 2, 0);
          if (translateX) {
            this.dom.content.style.left = `${translateX}px`;
          }
          else {
