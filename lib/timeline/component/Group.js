 ...
 */
  constructor(groupId, data, itemSet) {
    ...
    // [XAM]-S Enableing grouEditable on each group level
    this.groupEditable = {
      add: itemSet.options.groupEditable.add,
      order: itemSet.options.groupEditable.order,
      remove: itemSet.options.groupEditable.remove
    };
    // [XAM]-E
    ...
    // [XAM]-S Enableing grouEditable on each group level
    // update groupEditable
    if (data) {
      if (typeof data.groupEditable === 'boolean') {
        this.groupEditable = {
          add: data.groupEditable,
          order: data.groupEditable,
          remove: data.groupEditable
        };
      } else if (typeof data.groupEditable === 'object') {
        // TODO: in timeline.js 5.0, we should change this to not reset options from the timeline configuration.
        // Basically just remove the next line...
        this.groupEditable = {};
        util.selectiveExtend(['add', 'order', 'remove'], this.groupEditable, data.groupEditable);
      }
    }
    // [XAM]-E Enableing grouEditable on each group level
  ...
  _create() {
    ...
    if (this.groupEditable.order) { // [XAM] Enableing grouEditable on each group level
